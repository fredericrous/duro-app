...
never `--max-warnings`. The `duro` CLI prints a version-skew line on stderr
("docs are for @duro-app/ui@3.x, you have 1.0.0") while the UI packages stay
on 1.x; that is expected until the UI migration.
