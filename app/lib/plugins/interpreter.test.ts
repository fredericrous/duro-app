  ...
  const svc: PluginServices = {
    lldap: {
      addUserToGroup: (userId, groupName) =>
        Effect.sync(() => {
          calls.push({ method: "addUserToGroup", args: [userId, groupName] })
        }),
      removeUserFromGroup: (userId, groupName) =>
        Effect.sync(() => {
          calls.push({ method: "removeUserFromGroup", args: [userId, groupName] })
        }),
    ...
    http: {
      get: (url, opts) =>
        Effect.sync(() => {
          calls.push({ method: "http.get", args: [url, opts] })
          return {}
        }),
      post: (url, body, opts) =>
        Effect.sync(() => {
          calls.push({ method: "http.post", args: [url, body, opts] })
          return {}
        }),
      put: (url, body, opts) =>
        Effect.sync(() => {
          calls.push({ method: "http.put", args: [url, body, opts] })
          return {}
        }),
      del: (url, opts) =>
        Effect.sync(() => {
          calls.push({ method: "http.delete", args: [url, opts] })
        }),
    },
    vault: {
