...
import { parseAdminApplicationMutation, handleAdminApplicationMutation } from "~/lib/mutations/admin-application.server"
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  // can still be hit independently (single-fetch `?_routes=`) — self-gate.
...
export async function action({ request, params }: Route.ActionArgs) {
  // Shared admin-action gate: admin authz + strict Origin (a mutation with a
  // missing Origin header is now denied, where the old inline check let it
  // through — see admin-guard.server.ts).
  const auth = await requireAdminAction(request)

  const actor = await runEffect(
    requirePrincipal(auth.sub!).pipe(
      Effect.catchTag("PrincipalNotFound", () => Effect.succeed(null)),
      Effect.orDie,
    ),
  )
  if (!actor) throw new Response("Principal not found for current session", { status: 403 })

  const parsed = parseAdminApplicationMutation(await request.formData())
  if ("error" in parsed) return parsed

  // CRUD intents propagate infra failures to the error boundary (orDie);
  // syncNow/createGrant already map their failures to result values inside.
  return await runEffect(handleAdminApplicationMutation(params.id, actor, parsed).pipe(Effect.orDie))
}
            ...
            <Stack gap="md">
              ...
              {pendingRequests.length === 0 &&
                (application.accessMode === "request" || application.accessMode === "invite_only") && (
                  <Callout variant="success">{t("admin.applications.caughtUp")}</Callout>
                )}
              <AppOverview
