...
import { syncLogin } from "~/lib/governance/login-sync.server"
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  // a user's grants materialize on login, so it must always run) and record
  // the auth.login audit event.
  await runEffect(
    syncLogin(user, request.headers.get("x-forwarded-for") ?? undefined).pipe(
